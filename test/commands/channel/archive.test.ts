...
describe('channel archive', () => {
  ...
  it('archives a channel with --json flag', async () => {
    nock('https://slack.com')
      .post('/api/conversations.archive')
      .reply(200, {ok: true})

    const {stdout, exit} = await runCommand(['channel', 'archive', 'C123', '--json'])
    expect(exit).toBe(0)
  ...
  it('exits 3 on channel not found', async () => {
    ...
    const {exit} = await runCommand(['channel', 'archive', 'C999', '--json'])
    expect(exit).toBe(3)
  })
  ...
  it('exits 2 on auth error', async () => {
    ...
    const {exit} = await runCommand(['channel', 'archive', 'C123', '--json'])
    expect(exit).toBe(2)
  })
